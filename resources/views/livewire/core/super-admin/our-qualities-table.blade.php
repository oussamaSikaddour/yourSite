<div class="table__container" x-on:update-our-qualities-table.window="$wire.$refresh()">
    ...
    <div class="table__filters" wire:ignore>
        <div class="form__container">
            <form class="form">
                <div class="row">
                    <x-core.form.input model="name" :label="__('tables.our_qualities.name')" type="text" html_id="tName" role="filter" />
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <x-core.table.sortable-th wire:key="oqT-TH-2" model="name_fr" :label="__('tables.our_qualities.name')" :$sortDirection
                            :$sortBy />

                        <x-core.table.sortable-th wire:key="crt-TH-5" model="is_active" :label="__('tables.our_qualities.status')"
                            :$sortDirection :$sortBy />

                        <x-core.table.sortable-th wire:key="crt-TH-6" model="created_at" :label="__('tables.our_qualities.created_at')"
                            :$sortDirection :$sortBy />
                    ...
                    @foreach ($this->ourQualities as $oQ)
                        <tr wire:key="{{ $oQ->id }}-pr">
                            ...
                            <td>
                                ...
                                <livewire:core.table-selector wire:key="ts-oQ-{{ $oQ->id }}" :data="$stateOptions"
                                    :selectedValue="$oQ->state" :entityId="$oQ->id" lazy />
                            ...
                            <td>
                                <x-core.button variant="danger" icon="delete" function="openDeleteDialog"
                                    :parameters="[$oQ]" rounded=true hasTooltip=true :tooltip="__('toolTips.our_quality.delete')" />
                                ...
                                <livewire:core.open-modal-button wire:key="edit-oQ-{{ $oQ->id }}" rounded=true
                                    ...
                                    modalTitle="modals.our_quality.actions.update" :modalContent="[
