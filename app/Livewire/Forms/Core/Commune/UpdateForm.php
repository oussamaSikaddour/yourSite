...
class UpdateForm extends Form
{
    ...
    public function save($commune)
    {
        try {
            ...
            $commune->update($data);
            return $this->response(true, message: __("forms.commune.responses.update_success"));
        } catch (\Illuminate\Validation\ValidationException $e) {
            return $this->response(false, errors: $e->validator->errors()->all());
        } catch (\Exception $e) {
            Log::error('Error in Commune UpdateForm save method: ' . $e->getMessage());
