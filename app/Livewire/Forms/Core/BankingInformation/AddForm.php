...
class AddForm extends Form
{
    ...
    public function save()
    {
        try {
            ...
            BankingInformation::create($data);

            return $this->response(true, message: __("forms.banking_information.responses.success"));
        } catch (\Illuminate\Validation\ValidationException $e) {
            return $this->response(false, errors: $e->validator->errors()->all());
        } catch (\Exception $e) {
            Log::error('Error in BankingInformation AddForm save method: ' . $e->getMessage());
